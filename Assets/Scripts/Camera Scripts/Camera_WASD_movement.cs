...
public class Camera_WASD_movement: MonoBehaviour
{
    ...
    public int MovementSpeed = 50;        //Basic camera movement speed; default is "50".
    public int ShiftKeyBoost = 3;         //Multipler for camera movement speed when "Shift" key is pressed. Default is "3".
    public int CtrlKeySlowdown = 3;      //Slowdown multipler for camera movement when "Space" key is pressed. Default is "3"
    public int MouseSensitivity = 80;    //Than more that value than more sensitive mouse movement; default is "100".
 ...
 //Hide cursor in "Play" mode after losing viewport focus and bringing back cursor again; only inside Unity editor.
        ...
        if (!MasterScript.inspecting)
        {
            if (Input.GetKey(KeyCode.LeftShift) || Input.GetKey(KeyCode.RightShift))
            {
                ...
                if (Input.GetKey(KeyCode.Space)) { transform.position += transform.up * (MovementSpeedNormalized / 3) * ShiftKeyBoost; }
            }
            else
            {
                ...
                if (Input.GetKey(KeyCode.Space)) { transform.position += transform.up * MovementSpeedNormalized / 3; }
            }
            if (Input.GetKey(KeyCode.LeftControl))
            {
                transform.position += transform.forward * Input.GetAxis("Vertical") * (MovementSpeedNormalized / CtrlKeySlowdown);
                transform.position += transform.right * Input.GetAxis("Horizontal") * (MovementSpeedNormalized / CtrlKeySlowdown);
                if (Input.GetKey(KeyCode.Q)) { transform.position -= transform.up * (MovementSpeedNormalized / 3) / CtrlKeySlowdown; }
                if (Input.GetKey(KeyCode.Space)) { transform.position += transform.up * (MovementSpeedNormalized / 3) / CtrlKeySlowdown; }
            }
            else
            {
                transform.position += transform.forward * Input.GetAxis("Vertical") * MovementSpeedNormalized;
